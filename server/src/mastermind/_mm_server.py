...
class MastermindServerCallbacksDebug(object):
    def callback_connect(self):
        print(
            "Server: The server has connected to "
            + str(self._mm_ip)
            + " on port "
            + str(self._mm_port)
        )
    ...
    def callback_connect_client(self, connection_object):
        print(
            'Server: A new client from "'
            + str(connection_object.address)
            + '" has connected.'
        )
        return super(MastermindServerCallbacksDebug, self).callback_connect_client(
            connection_object
        )

    def callback_disconnect_client(self, connection_object):
        print(
            'Server: The client from "'
            + str(connection_object.address)
            + '" has disconnected.'
        )
        return super(MastermindServerCallbacksDebug, self).callback_disconnect_client(
            connection_object
        )

    def callback_client_receive(self, connection_object):
        print(
            'Server: About to receive information from client "'
            + str(connection_object.address)
            + '"!'
        )
        return super(MastermindServerCallbacksDebug, self).callback_client_receive(
            connection_object
        )

    def callback_client_handle(self, connection_object, data):
        print(
            'Server: Handling data "'
            + str(data)
            + '" from client "'
            + str(connection_object.address)
            + '".'
        )
        return super(MastermindServerCallbacksDebug, self).callback_client_handle(
            connection_object, data
        )

    def callback_client_send(self, connection_object, data, compression=None):
        print(
            'Server: About to send data "'
            + str(data)
            + '" to client "'
            + str(connection_object.address)
            + '" with compression "'
        ...
        return super(MastermindServerCallbacksDebug, self).callback_client_send(
...
class MastermindServerCallbacksEcho(object):
    def callback_client_handle(self, connection_object, data):
        ...
        return super(MastermindServerCallbacksEcho, self).callback_client_handle(
            connection_object, data
        )
